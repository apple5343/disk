...
**Важно**: Данный проект является учебным. Этот репозитоий направлен на демонстрацию работы сервисов **file-storage-service** и **file-data-service**.
...
- [Основне сценарии](#основне-сценарии)
...
- [Вспомогательные компоненты](#вспомогательные-компоненты)
  - [Zookeeper](#zookeeper)
  - [Nginx](#nginx)
  - [Как пользоваться API через Swagger UI](#как-пользоваться-api-через-swagger-ui)
...
3. Проверяется начичие процесса загрузки в Redis.
...
2. Проверяется начичие процесса загрузки в Redis.
...
5. Процессу загрузки посылается сигнал об отмене (реализовано с помощью отмены контекста). Происходит остановка процесса: 
   ...
   1. Прерывется хагрузка файла в MinIO.
